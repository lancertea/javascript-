...
JavaScript 代码在执行之前会先经过编译阶段，包括词法分析、语法分析、生成 AST，并创建执行上下文；随后进入执行阶段，代码自上而下执行，在执行上下文中通过作用域链查找变量，同时执行上下文由执行上下文栈进行管理。在代码运行过程中，如果某些代码被频繁执行，JIT 编译器会把这些热点代码优化为机器码，以提高执行效率，同时 JavaScript 引擎还会通过垃圾回收机制自动管理内存
