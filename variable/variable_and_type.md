...
- 基本类型直接存储值，占用空间固定，存储在栈内存（stack）  
- 引用类型存储的是对象，对象存储在堆内存（heap），变量中保存的是地址（引用）
为什么引用类型放在堆中：
- 对象大小不固定，如果存在栈里，每次扩容都要重新分配空间，会影响栈结构，放在堆内存更灵活
- 栈空间很小，一般只有几MB，如果对象放栈里，很容易造成栈溢出
- 栈需要高速访问，如果把复杂对象放进去，会降低访问效率
...
基本类型是没有属性和方法的，但仍然可以使用对象才有的属性方法。这时因为在对基本类型使用属性方法的时候，JS做了隐式包装，会隐式地创建这个基本类型的对象（即基本包装类型），之后再销毁这个对象
1 创建临时 String 对象
2 调用方法
3 销毁临时对象
