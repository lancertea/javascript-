...
while (i < btns.length) {
  ...
  // ==========================================
  // 3. 核心秘密：这里有一个隐藏的“块级作用域”
  // 每次循环，引擎都会在这里新建一个独立的环境
  { 
    // 4. 把外部的 i 的值，复制给当前块级作用域里一个新的 i
    // 注意：这个 i 和外面的 i 虽然同名，但其实是两个独立的变量！
    let i = i; 
    
    // 5. 绑定事件（此时绑定的函数，闭包记住的是块级作用域里的 i）
    btns[i].onclick = function() {
      console.log("第" + i + "个按钮被点击")
    }
  }
...
### 箭头函数和普通函数的区别
1. 箭头函数语法更简洁，特别适合写 **回调函数**（ES6中每一种函数都可以使用形参赋默认值和剩余运算符）
2. 普通函数的函数声明会变量提升，箭头函数的创建都是函数表达式方式（变量=函数），只能在创建完成后被执行
...
- 没有自己的 this
- 没有 prototype、constructor，而 new 命令在执行时需要将构造函数的 prototype 赋值给新的对象的__proto__
